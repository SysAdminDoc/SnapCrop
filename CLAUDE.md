...
## Version History
- v6.5.5: **Fifth-pass audit (11 fixes).** **HomeScreen version hardcoded** — HomeScreen header displayed hardcoded "v6.5.0" despite SettingsActivity being fixed in v6.5.3; now uses `BuildConfig.VERSION_NAME`. **saveCropped bitmap leak** — if `saveToGallery` or `applyBorder`/`applyWatermark` threw an exception, the cropped bitmap was never recycled; refactored to declare bitmap outside try and always recycle in finally with `isRecycled` guard. **PDF export orphaned MediaStore row** — `exportPdf` inserted a MediaStore row with `IS_PENDING=1` but if `openOutputStream` returned null, the row was left dangling; now deletes the row and throws to trigger the error toast. **Pixelate missing undo** — adding a pixelate region did not call `pushUndo()`, making pixelate operations impossible to undo via the global undo system; fixed by adding pushUndo before `pixelateRects.add()`. **applyTiltShift dead code** — removed unused `Canvas(result)` allocation in tilt-shift processing (the function uses pixel arrays, not Canvas drawing). **Palette stale on bitmap change** — color palette was not reset when bitmap changed (rotation, BG removal, resize), showing old colors; LaunchedEffect(bitmap) now clears paletteColors. **Favorite videos missing duration** — `loadFavoritePhotos` used a single projection without DURATION column; videos in Favorites showed 0:00; split projection for video queries to include duration. **PDF export OOM** — large images (50+ MP) were decoded at full resolution for PDF pages, risking OOM; now downscales to max 2048px dimension before adding to PdfDocument. **Spotlight zero-size guard** — spotlight tool with identical start/end points (degenerate tap) produced a fully-dimmed image; added guard to skip zero-size spotlights. **Callout text centering** — callout numbered circles used empirical `y + radius * 0.4f` for text positioning; replaced with fontMetrics-based true vertical centering. **Emoji baseline centering** — emoji overlay drew at raw baseline causing emoji to appear too high; now uses fontMetrics for proper vertical center alignment.
- v6.5.4: **Deep audit agent findings (10+ fixes).** **copyToClipboard double recycle** — if `withContext(Dispatchers.Main)` threw after `cropped.recycle()`, the catch block recycled again; now uses `finally` with `isRecycled` guard. **Draw coordinate clamping** — draw/tap coordinates clamped to `(width-1, height-1)` instead of `(width, height)` preventing out-of-bounds pixel access in CropActivity export. **Duration formatting** — video durations over 1 hour now display as `H:MM:SS` instead of `61:01`. **Size formatting** — file sizes use 1-decimal MB display (`1.5MB` instead of `1MB`). **Settings quality coercion** — jpeg_quality and target_size_kb values from SharedPreferences are clamped to valid slider ranges on load (prevents stale values from older app versions). **Filename sanitization** — filename template in Settings now strips filesystem-reserved characters (`<>:"/\|?*`). **StitchActivity list snapshot** — image URI list is snapshotted before save to prevent race with UI reordering during IO. **SmartCropEngine 5s timeout** — ML Kit object detection now has a 5-second timeout via `withTimeoutOrNull`, preventing indefinite suspension on slow devices. **AutoCrop small image guard** — images under 100px in either dimension skip border detection (returns full rect) since sampling creates overlapping coordinates.
- v6.5.3: **Third-pass deep audit (20+ fixes).** **ML Kit cancellation safety** — all 5 ML Kit wrappers (BackgroundRemover, SmartCropEngine, FaceDetector, BarcodeScanner, TextExtractor) now check `cont.isActive` before resuming `suspendCancellableCoroutine`, preventing crash when coroutine is cancelled while ML Kit processes asynchronously. **BackgroundRemover isRecycled guard** — added missing recycled bitmap check (other ML Kit wrappers had it). **Stitch/Collage/DeviceFrame save off main thread** — `saveToGallery` was called via `withContext(Dispatchers.Main)` but performs IO (openOutputStream, compress); now runs on IO with Toasts via `runOnUiThread`. **StitchActivity bitmap leak** — when fewer than 2 images decode successfully, the decoded bitmaps were not recycled before early return. **Adjust Reset includes curves** — Reset button in ADJUST mode now also resets curveR/curveG/curveB to 0 (was leaving curve values unchanged). **Undo for tap-place draw tools** — Callout, Magnifier, Emoji, and Text tools now call `pushUndo()` before placing (were not undoable). **Settings version from BuildConfig** — replaced hardcoded `"SnapCrop v6.5.0"` with `BuildConfig.VERSION_NAME`; enabled `buildConfig = true` in build.gradle.kts. **Cache clear off main thread** — `cacheDir.deleteRecursively()` in Settings now runs on background thread.
- v6.5.2: **Second-pass audit fixes.** **Orphaned MediaStore cleanup** — batchAutocrop/batchResize now delete the MediaStore row when openOutputStream returns null (prevents ghost entries with IS_PENDING=0 and empty files). **PhotoGrid O(n) indexOf fix** — date-header mode pre-computes `indexById` map for O(1) lookup instead of O(n) `photos.indexOf()` per item. **Viewer per-page zoom** — zoom/pan state is now per-page in HorizontalPager (previously shared state caused adjacent pages to appear zoomed when swiping). **ScreenshotService thumbnail recycle removed** — removed premature `thumbnail?.recycle()` after `nm.notify()` that could crash when notification system reads bitmap asynchronously. **Dead variable removed** — `preSnapshotTime` in delayed capture was assigned but never read. **AutoCrop method label fix** — redundant `"border"` on both branches now correctly returns `"border+bars"` when system bars were also stripped. **CollageActivity save guard** — save button requires 2+ images (was 1+, but collage with 1 image is meaningless).
- v6.5.1: **Comprehensive bug audit (50+ fixes).** **Lifecycle-safe coroutines** — replaced all unstructured `CoroutineScope` with `lifecycleScope`/`rememberCoroutineScope` across CropActivity, MainActivity, GalleryScreen, StitchActivity, CollageActivity, DeviceFrameActivity (prevents crashes on config change and memory leaks). **saveToGallery off main thread** — CropActivity save pipeline fully on IO dispatcher, Toasts via `runOnUiThread` (prevents ANR). **isSaving reset on failure** — saveCropped wrapped in try-finally so saving overlay never gets stuck. **finish() only on success** — save failure no longer closes activity, user can retry. **Bitmap lifecycle fixes** — createCroppedBitmap no longer recycles returned bitmap when `cropped === drawn`; applyTiltShift input recycled by caller; shareCropped double-recycle eliminated (finally block); heal tool uses read-only source array to prevent progressive smearing; floodFill ArrayDeque initial capacity reduced from `w*h/4` to 1024 (prevents OOM). **NaN zoom guard** — prevSpread threshold raised from 0 to 1 to prevent NaN corruption of zoomLevel. **DrawScope state writes** — baseScale/baseOx/baseOy only written when changed (prevents recomposition loop). **O(n^2) draw points** — currentDrawPoints changed from immutable list concatenation to mutableStateListOf with .add(). **Vignette Paint hoisted** — Paint object remembered instead of allocated every frame. **Draw undo stack** — draw-mode undo/redo now pushes to global undo stack. **bgRemoving reset** — LaunchedEffect resets on bitmap change. **Eyedropper guard** — checks `bitmap.isRecycled` before getPixel. **Auto-enhance zero guard** — sw/sh clamped to minimum 1. **Draw tool row scrollable** — horizontalScroll added to prevent clipping on narrow screens. **GalleryScreen viewer sort fix** — viewer now receives sorted photos matching grid order. **Favorites loads videos** — loadFavoritePhotos queries both Images and Video MediaStore. **SQL >999 fix** — favorite IDs batched in chunks of 500 for SQLite bind limit. **loadAlbums includes videos** — album counts and covers now reflect both images and videos. **DeviceFrameActivity format preference** — now respects save format, quality, and save path settings (was hardcoded PNG). **Main-thread delete fix** — MANAGE_EXTERNAL_STORAGE delete loop moved to IO dispatcher. **PdfDocument leak fix** — doc.close() in finally block. **OEM intent crash fix** — MANAGE_APP_ALL_FILES_ACCESS_PERMISSION wrapped in try-catch with fallback. **serviceRunning accuracy** — UI toggle reflects actual service state only. **Batch crop zero-size guard** — crop width/height clamped to minimum 1. **ScreenshotService quickSave off main thread** — wrapped in coroutine on IO. **Bitmap leak in quickSave** — try-finally ensures both bitmaps recycled. **Notification thumbnail recycled** after nm.notify(). **AutoCrop borderColor for left/right edges** — findLeftEdge/findRightEdge now accept and check borderColor (fixes dark mode vertical border detection). **MonitorTileService API 34** — startActivityAndCollapse uses PendingIntent on API 34+. **CollageActivity guards** — cellW/cellH minimum 1, requires 2+ images. **Stitch/Collage/DeviceFrame openOutputStream null** — explicit null check throws IOException. **ML Kit recycled bitmap guards** — SmartCropEngine, FaceDetector, BarcodeScanner, TextExtractor all check isRecycled before processing.
