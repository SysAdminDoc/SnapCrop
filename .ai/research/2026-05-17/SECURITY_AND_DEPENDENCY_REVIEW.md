...
|---|---|---|---|
| Android Gradle Plugin | 8.7.3 | Google Maven metadata showed latest stable 9.2.1 and latest alpha 9.3.0-alpha05. | Upgrade in a controlled P0 dependency pass after CI baseline exists. |
| Kotlin Gradle Plugin | 2.0.21 | Maven Central metadata showed latest stable 2.3.21 and RC 2.4.0-RC. | Upgrade with AGP/Compose compatibility checks. |
| Compose BOM | 2024.12.01 | Google Maven metadata showed 2026.05.00. | Upgrade with visual/manual editor smoke checks. |
| core-ktx | 1.15.0 | Metadata showed 1.18.0. | Upgrade with compile/lint checks. |
| activity-compose | 1.9.3 | Metadata showed 1.13.0. | Upgrade and verify Activity Result, edge-to-edge, back behavior. |
| navigation-compose | 2.8.5 | Metadata showed 2.9.8. | Upgrade with navigation smoke tests. |
| lifecycle-runtime-ktx | 2.8.7 | Metadata showed 2.10.0. | Upgrade with lifecycle/coroutine checks. |
...
Current workflow only runs manual `assembleRelease` and uploads an artifact.

Recommended additions:

- PR and push triggers for build/lint/test.
- `actions/dependency-review-action` on pull requests.
- CycloneDX Gradle SBOM generation for release artifacts.
...
|---|---|---|---|
| `MANAGE_EXTERNAL_STORAGE` | High Play policy scrutiny | Prompt-free source screenshot deletion/replacement. | Document user benefit, keep feature optional, make fallback behavior explicit. |
| `SYSTEM_ALERT_WINDOW` | User trust and platform restriction risk | Background launch/overlay-like screenshot editor flow. | Ensure notification fallback is primary when background launch is blocked. |
| AccessibilityService | High user trust sensitivity | Long screenshot capture and scroll automation. | Keep clear in-app explanation, no unrelated accessibility data use, easy disable path. |
| `FOREGROUND_SERVICE_SPECIAL_USE` | Android 14+ declaration scrutiny | Screenshot monitoring service. | Verify subtype property and Play Console wording. |
...
| `android:allowBackup="true"` | Potential privacy metadata backup | Default Android backup enabled. | Audit SharedPreferences and exclude sensitive automation/profile/recent metadata if needed. |
...
1. Add `SECURITY.md` with privacy, permissions, reporting, and release policy.
2. Add CI dependency review and SBOM.
3. Audit backup behavior and sensitive preferences.
4. Add permission matrix to README.
5. Add export regression tests for stale-byte/redaction safety.
6. Review tracked `.idsig` artifacts and remove if not intentionally versioned.
